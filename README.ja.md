...
5. StickS3 をカメラ前面の赤外線受光部に向けます（最適距離は約 40cm です）。
6. **操作方法:**
   - **BtnB (正面 M5ボタン):** カウントダウンの開始/キャンセルを行います。
   - **BtnA (側面ボタン):** クリックでタイマー/露光時間を切り替えます。長押しで **Shot モード** と **Bulb モード** を切り替えます。
...
Lomoの赤外線プロトコルをどのようにデコードし、リバースエンジニアリングして検証したかについて興味がある方は、[docs/](docs/) ディレクトリをご覧ください。メインの README は意図的に短くしています — このリポジトリのコードはすでに「すぐに使える」状態です！
